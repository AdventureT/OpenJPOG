 ...
 */
 ...
 // !!! FIXME: can we move this off THEORAPLAY_sAllocate to a custom allocator?
...
static void* THEORAPLAY_DefaultAllocator(size_t size)
{
    return malloc(size);
}

static void THEORAPLAY_DefaultDeallocator(void* ptr)
{
    free(ptr);
}

void THEORAPLAY_SetMemoryFunctions(THEORAPLAY_tAllocator allocator, THEORAPLAY_tDeallocator deallocator)
{
    THEORAPLAY_sAllocate = allocator;
    THEORAPLAY_sDeallocate = deallocator;
}

THEORAPLAY_tAllocator THEORAPLAY_sAllocate = THEORAPLAY_DefaultAllocator;
THEORAPLAY_tDeallocator THEORAPLAY_sDeallocate = THEORAPLAY_DefaultDeallocator;
    ...
    const int p0, const int p1, const int p2)
{
    ...
    unsigned char* yuv = (unsigned char*)THEORAPLAY_sAllocate(w * h * 2);
        ...
        while (!ctx->halt && vpackets)
        {
            ...
            if (frames > 0)
            {
                if (!resolving_audio_seek)
                {
                    ...
                    float* samples;
                    AudioPacket* item = (AudioPacket*)THEORAPLAY_sAllocate(sizeof(AudioPacket));
                    ...
                    item->frames = frames;
                    item->samples = (float*)THEORAPLAY_sAllocate(sizeof(float) * frames * channels);
                    item->next = NULL;

                    if (item->samples == NULL)
                    {
                        THEORAPLAY_sDeallocate(item);
                        goto cleanup;
                    } // if
            ...
            else
            {
                ...
                if (th_decode_packetin(tdec, &packet, &granulepos) == 0)  // new frame!
                {
                    ...
                    if (!resolving_video_seek)
                    {
                        ...
                        {
                            VideoFrame* item = (VideoFrame*)THEORAPLAY_sAllocate(sizeof(VideoFrame));
                            if (item == NULL) goto cleanup;
                            item->seek_generation = current_seek_generation;
                            item->playms = playms;
                            item->fps = fps;
...
THEORAPLAY_Decoder* THEORAPLAY_startDecodeFile(const char* fname,
    ...
    THEORAPLAY_VideoFormat vidfmt)
{
    THEORAPLAY_Io* io = (THEORAPLAY_Io*)THEORAPLAY_sAllocate(sizeof(THEORAPLAY_Io));
    if (io == NULL)
        return NULL;
...
#undef VIDCVT
    ...
    ctx = (TheoraDecoder*)THEORAPLAY_sAllocate(sizeof(TheoraDecoder));
    if (ctx == NULL)
...
void THEORAPLAY_stopDecode(THEORAPLAY_Decoder* decoder)
{
    ...
    while (videolist)
    {
        ...
        THEORAPLAY_sDeallocate(videolist->pixels);
        THEORAPLAY_sDeallocate(videolist);
        videolist = next;
    } // while
    ...
    while (audiolist)
    {
        ...
        THEORAPLAY_sDeallocate(audiolist->samples);
        THEORAPLAY_sDeallocate(audiolist);
        audiolist = next;
    } // while
...
void THEORAPLAY_freeAudio(const THEORAPLAY_AudioPacket* _item)
{
    ...
    if (item != NULL)
    {
        ...
        THEORAPLAY_sDeallocate(item->samples);
        THEORAPLAY_sDeallocate(item);
    } // if
...
void THEORAPLAY_freeVideo(const THEORAPLAY_VideoFrame* _item)
{
    ...
    {
        assert(item->next == NULL);
        THEORAPLAY_sDeallocate(item->pixels);
        THEORAPLAY_sDeallocate(item);
    } // if
